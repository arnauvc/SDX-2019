...
node(MyKey, Predecessor, Successor,Store,Next,Replica) ->
    receive 
		    delete ->
			      io:format("Delete OK"),
			      node(MyKey, Predecessor, Successor,[],Next,Replica);
        {key, Qref, Peer} ->
            Peer ! {Qref, MyKey},
            node(MyKey, Predecessor, Successor, Store,Next,Replica);
        {notify, NewPeer} ->
		...
		{replicate, Key, Value} ->
			NewReplica = lists:keystore(Key,1,Replica,{Key, Value}),
        ...
        {'DOWN', Ref, process, _, _} ->
			...
			true -> 
				{_,_,Spid} = NewSucc,
				Spid ! {pushreplica, Store},
            	node(MyKey, NewPred, NewSucc, Store, NewNext,Replica)
			end
