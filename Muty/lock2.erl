...
wait(Nodes, Master, Refs, Waiting, TakeRef, MyId) ->
    receive
	    ...
	    if
		MyId > OtherId -> From ! {ok, Ref},R = make_ref(),From ! {request, self(), R, MyId}, wait(Nodes, Master, [R | Refs], Waiting, TakeRef, MyId);

		MyId =< OtherId -> wait(Nodes, Master, Refs, [{From, Ref}|Waiting], TakeRef, MyId)
	    end;
        {ok, Ref} ->
